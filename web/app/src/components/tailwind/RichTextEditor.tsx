...
export default function RichTextEditor() { // we could use a lib here but as the actual purpose of this project is to showcase my knowledge lets go with this
    ...
    return (
        <div className="w-full flex flex-col relative">
            <input
                ...
                onChange={
                    (e) => {
                        ...
                        else if (lastToken.trim().length === 0) { // if is a whitespace or ln
                        ...
                        switch (e.code) {
                            case "Tab":
                                ...
                                if (e.shiftKey) {
                                    ...
                                    setPaneSelected(Math.min(paneSelected + 1, (filteringObjects ? objects.length : users.length) - 1))
                                }
                                break
                            case "ArrowDown":
                                e.preventDefault()
                            ...
                            case "Enter":
                                e.preventDefault()
                                if (filteringUsers) {
                                    setValue(value.slice(0, value.lastIndexOf("@") + 1) + users[paneSelected])
                                    setFilteringUsers(false)
                                } else if (filteringObjects) {
                                    setValue(value.slice(0, value.lastIndexOf("#") + 1) + objects[paneSelected])
                                    setFilteringObjects(false)
                                }
                                break
                            default:
            ...
            {
                filteringObjects ? (
                    <RichTextCompletePane selected={paneSelected} query={filteringQuery} inputRef={inputRef} elements={objects}></RichTextCompletePane>
                ) : filteringUsers ? (
                    <RichTextCompletePane selected={paneSelected} query={filteringQuery} inputRef={inputRef} elements={users}></RichTextCompletePane>
                ) : null
            }
        </div>
    )
